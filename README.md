...
6. Выбрать «Живую смену» — приложение выдаст новый код вида `PL-7K42`,
   ...
   и расписание событий. Смену можно переиграть по тому же коду или
   передать ссылкой.
7. Открыть вкладку **AI Worklog** — отчёт о том, как приложение делалось.
...
и ту же директиву. Код показан мелкой подписью под переключателем режима и
сам попадает в адресную строку, так что смену можно отправить, ничего не
нажимая.
...
**165 тестов в тринадцати файлах.** Доменные идут в среде `node` — логика не должна
...
- [x] `npm test` проходит — 165 тестов
