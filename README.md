...
**199 тестов в пятнадцати файлах.** Доменные идут в среде `node` — логика не должна
...
production preview в браузере. Опубликованную версию я смотрел и на телефоне —
именно там нашлось перекрытие подсказки вступления. Скринридером приложение
никто не слушал, отмечено честно:
...
- [x] `npm test` проходит — 199 тестов
...
| Израсходованные токены | «Коротко о проекте» — около 717 тысяч output tokens |
