...
**195 тестов в пятнадцати файлах.** Доменные идут в среде `node` — логика не должна
...
- [x] `npm test` проходит — 195 тестов
...
Встроен в приложение отдельной вкладкой — как требует задание. Время разработки
указано как **около 12–13 часов за полтора рабочих дня**, расход — **около 717
тысяч output tokens** по счётчику Mirasim за 21–22 сентября. Тот же текст лежит
в [`docs/worklog.md`](docs/worklog.md): файл и экран не имеют права
расходиться.
