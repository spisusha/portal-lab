...
2. Сразу под целью смены — блок **«Требует решения сейчас»**: приложение само
   выбирает самый опасный портал, объясняет, почему именно его, и предлагает
   одно главное действие. Разбираться, с какой строки начинать, не нужно.
3. Нажать «Стабилизировать» прямо в этом блоке — риск пересчитается и упадёт,
   изменение попадёт в журнал и в историю портала.
4. Переключить смену на «Критическую ситуацию» (селектор в шапке) — там есть
...
src/ui/        компоненты — только отрисовка
  ...
  PortalRing.tsx живой портал: шкала опасности формой, а не только цветом
```
...
**49 тестов в шести файлах.** Доменные идут в среде `node` — логика не должна
...
- блок «требует решения» сам выбирает нужный портал и объясняет выбор;
- главная кнопка выполняет действие, риск падает 72 → 60, событие в журнале;
- при ранге S отправка наблюдателя недоступна, причина видна рядом;
- закрытие портала с существами требует подтверждения и отменяется без следов;
- цикл двигает часы и пишет в журнал;
- пустая смена объясняет себя и даёт выход;
- до главных кнопок можно дойти табом, диалоги закрываются по Escape.

## Чеклист проверки перед сдачей

Обязательные состояния из ТЗ. Все они прогнаны **автоматизированно** —
браузером через CDP на живой ссылке и тестами; ручного прохода человеком
пока не было, отмечено честно:

- [x] **Пустой список порталов** — сценарий «Пустая лаборатория»: вместо
      пустого экрана сообщение и кнопка возврата к данным, а блок «требует
      решения» сообщает, что смена спокойна.
...
- [x] нет горизонтальной прокрутки на 360, 390, 768 и 1280 px
